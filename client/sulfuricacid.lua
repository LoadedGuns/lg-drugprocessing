...
local function SpawnSulfuricAcidBarrels()
	...
	while spawnedSulfuricAcidBarrels < 30 do
		Wait(0)
...
RegisterNetEvent("lg-drugprocessing:pickSulfuric", function()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				spawnedSulfuricAcidBarrels -= 1
				TriggerServerEvent('lg-drugprocessing:pickedUpSulfuricAcid')
				isPickingUp = false
			end, function()
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
