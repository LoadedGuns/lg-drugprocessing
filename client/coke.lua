...
local function ProcessCoke()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processCocaLeaf')
...
local function CutCokePowder()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processCocaPowder')
...
local function ProcessBricks()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processCocaBrick')
...
local function BagCoke()
	isProcessing = true
	local playerPed = PlayerPedId()

	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.bagging_coke"), 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function()
		TriggerServerEvent('lg-drugprocessing:bagcoke')
		local timeLeft = Config.Delays.CokeProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
RegisterNetEvent('lg-drugprocessing:ProcessBricks', function()
...
-- RegisterNetEvent('lg-drugprocessing:EnterCWarehouse', function()
-- 	local ped = PlayerPedId()
...
-- RegisterNetEvent('lg-drugprocessing:ExitCWarehouse', function()
-- 	local ped = PlayerPedId()
-- 	local pos = GetEntityCoords(ped)
...
RegisterNetEvent('lg-drugprocessing:pickCocaLeaves', function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				TriggerServerEvent('lg-drugprocessing:pickedUpCocaLeaf')
				isPickingUp = false
			end, function()
				ClearPedTasks(playerPed)
				isPickingUp = false
