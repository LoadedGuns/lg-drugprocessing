...
local function SpawnSodiumHydroxideBarrels()
	...
	while spawnedSodiumHydroxideBarrels < 30 do
		Wait(0)
...
RegisterNetEvent("lg-drugprocessing:pickSodium", function()
	...
	if nearbyObject3 and IsPedOnFoot(playerPe3) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				TriggerServerEvent('lg-drugprocessing:pickedUpSodiumHydroxide')
				isPickingUp = false
			end, function()
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
