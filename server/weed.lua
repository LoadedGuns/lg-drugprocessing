...
RegisterServerEvent('lg-drugprocessing:pickedUpCannabis', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	if Player.Functions.AddItem("cannabis", math.random(1, 3)) then
...
RegisterServerEvent('lg-drugprocessing:processCannabis', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('lg-drugprocessing:rollJoint', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateUseableItem("rolling_paper", function(source, item)
    ...
    TriggerClientEvent('lg-drugprocessing:client:rollJoint', source, 'marijuana', item)
end)

RegisterServerEvent('lg-drugprocessing:bagskunk', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateUseableItem("empty_weed_bag", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('lg-drugprocessing:client:bagskunk', source, 'marijuana', item)
end)
