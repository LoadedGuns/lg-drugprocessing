...
local function ProcessChemicals()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processChemicals')
...
local function ProcessTempUp()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_up"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processTempUp')
...
local function ProcessTempDown()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_down"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processTempDown')
...
local function ProcessProduct()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('lg-drugprocessing:processMeth')
...
RegisterNetEvent('lg-drugprocessing:ProcessChemicals', function()
...
RegisterNetEvent('lg-drugprocessing:ChangeTemp', function()
	if not isTempChangeU then
		QBCore.Functions.TriggerCallback('lg-drugprocessing:validate_items', function(result)
			if result.ret then
				exports['lg-ui']:Thermite(function(success)
					if success then
						QBCore.Functions.Notify(Lang:t("success.temp_up"), 'success')
						ProcessTempUp()
					else
						TriggerServerEvent('lg-drugprocessing:cancelProcessing')
						TriggerServerEvent('lg-drugprocessing:processFailUp')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('lg-drugprocessing:ChangeTemp2', function()
			...
			if result.ret then
				exports['lg-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('lg-drugprocessing:cancelProcessing')
						TriggerServerEvent('lg-drugprocessing:processFailDown')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('lg-drugprocessing:ProcessProduct', function()
	local coords = GetEntityCoords(PlayerPedId(source))
...
-- RegisterNetEvent('lg-drugprocessing:EnterLab', function()
-- 	local ped = PlayerPedId()
...
-- RegisterNetEvent('lg-drugprocessing:ExitLab', function()
-- 	local ped = PlayerPedId()
-- 	local pos = GetEntityCoords(ped)
